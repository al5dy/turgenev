...
- `EditorIntegration` reports `highlightsAvailable` to the browser through `TurgenevConfig`, so the result panel's accordion sections render disabled instead of a Highlight action that is guaranteed to fail, with a short, non-fatal note instead.
- Balance and document analysis (`risk`) never touch `ReportHighlightParser` and are unaffected either way.
- `ReportSectionParser` shares the same `Requirements::hasDom()` gate: opening an accordion section fetches the provider's report page for that one section (`ApiClient::reportSectionDetails()`, the same read-only report form as `reportHighlights()`, plus a `coverdict` field selecting the tab) and degrades the same way when `ext-dom` is unavailable. Two report fields the provider only fills inside its own logged-in browser session — the per-document verdict sentence and the "problems in this sentence" preview — are always empty on this anonymous, token-only fetch, so neither is parsed; the overall verdict shown in the accordion comes from `RiskResult.level` (the JSON `risk` operation) instead.
